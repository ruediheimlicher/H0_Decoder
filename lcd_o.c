...
void lcd_o_command(uint8_t cmd[], uint8_t size) {
#if defined I2C
    i2c_start((LCD_I2C_ADR << 1) | 0);
    i2c_write_byte(0x00);    // 0x00 for command, 0x40 for data
    for (uint8_t i=0; i<size; i++) {
        i2c_write_byte(cmd[i]);
    }
    i2c_stop();
#elif defined SPI
	LCD_PORT &= ~(1 << CS_PIN);
...
void lcd_o_data(uint8_t data[], uint16_t size) {
#if defined I2C
    i2c_start((LCD_I2C_ADR << 1) | 0);
    i2c_write_byte(0x40);    // 0x00 for command, 0x40 for data
...
void lcd_o_init(uint8_t dispAttr){
#if defined I2C
    i2c_init();
#elif defined SPI
    ...
    lcd_o_command(commandSequence, sizeof(commandSequence));
    lcd_o_clrscr();
}
void lcd_o_gotoxy(uint8_t x, uint8_t y){
    x = x * sizeof(FONT[0]);
    lcd_o_goto_xpix_y(x,y);
}
void lcd_o_goto_xpix_y(uint8_t x, uint8_t y){
...
void lcd_o_clrscr(void){
#ifdef GRAPHICMODE
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        memset(displayBuffer[i], 0x00, sizeof(displayBuffer[i]));
        lcd_o_gotoxy(0,i);
        lcd_o_data(displayBuffer[i], sizeof(displayBuffer[i]));
    }
#elif defined TEXTMODE
    ...
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        lcd_o_gotoxy(0,i);
        lcd_o_data(displayBuffer, sizeof(displayBuffer));
    }
#endif
    lcd_o_home();
}
void lcd_o_home(void){
    lcd_o_gotoxy(0, 0);
}
void lcd_o_invert(uint8_t invert){
    ...
    lcd_o_command(commandSequence, 1);
}
void lcd_o_sleep(uint8_t sleep){
    uint8_t commandSequence[1];
    if (sleep != YES) {
...
void lcd_o_set_contrast(uint8_t contrast){
    uint8_t commandSequence[2] = {0x81, contrast};
    lcd_o_command(commandSequence, sizeof(commandSequence));
}
void lcd_o_putc(char c){
    switch (c) {
        case '\b':
            // backspace
            lcd_o_gotoxy(cursorPosition.x-charMode, cursorPosition.y);
            lcd_o_putc(' ');
            lcd_o_gotoxy(cursorPosition.x-charMode, cursorPosition.y);
            break;
        case '\t':
            ...
            if( (cursorPosition.x+charMode*4) < (DISPLAY_WIDTH/ sizeof(FONT[0])-charMode*4) ){
                lcd_o_gotoxy(cursorPosition.x+charMode*4, cursorPosition.y);
            }else{
                lcd_o_gotoxy(DISPLAY_WIDTH/ sizeof(FONT[0]), cursorPosition.y);
            }
            break;
        case '\n':
            // linefeed
            if(cursorPosition.y < (DISPLAY_HEIGHT/8-1)){
                lcd_o_gotoxy(cursorPosition.x, cursorPosition.y+charMode);
            }
            break;
        case '\r':
            // carrige return
            lcd_o_gotoxy(0, cursorPosition.y);
            break;
        default:
            ...
            if (charMode == DOUBLESIZE) {
                ...
                lcd_o_data(data, sizeof(FONT[0])*2);
...
void lcd_o_charMode(uint8_t mode){
    charMode = mode;
}
void lcd_o_flip(uint8_t flipping){
	...
	switch(flipping){
		case 0:
			...
			command[0] = 0xC8;
			command[1] = 0xA1;
			lcd_o_command(command, sizeof(command));
			break;
		case 1:
...
void lcd_o_puts(const char* s){
    while (*s) {
        lcd_o_putc(*s++);
    }
}
void lcd_o_puts_p(const char* progmem_s){
    register uint8_t c;
...
uint8_t lcd_o_drawPixel(uint8_t x, uint8_t y, uint8_t color){
...
uint8_t lcd_o_drawLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2, uint8_t color){
	uint8_t result;
    ...
    while(1){
        result = lcd_o_drawPixel(x1, y1, color);
...
uint8_t lcd_o_drawRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2, uint8_t color){
    uint8_t result;
    
    result = lcd_o_drawLine(px1, py1, px2, py1, color);
    result = lcd_o_drawLine(px2, py1, px2, py2, color);
    result = lcd_o_drawLine(px2, py2, px1, py2, color);
    result = lcd_o_drawLine(px1, py2, px1, py1, color);
    
    return result;
}
uint8_t lcd_o_fillRect(uint8_t px1, uint8_t py1, uint8_t px2, uint8_t py2, uint8_t color){
    uint8_t result;
    ...
    for (uint8_t i=0; i<=(py2-py1); i++){
        result = lcd_o_drawLine(px1, py1+i, px2, py1+i, color);
    }
    
    return result;
}
uint8_t lcd_o_drawCircle(uint8_t center_x, uint8_t center_y, uint8_t radius, uint8_t color){
    uint8_t result;
    ...
    result = lcd_o_drawPixel(center_x  , center_y+radius, color);
    result = lcd_o_drawPixel(center_x  , center_y-radius, color);
    result = lcd_o_drawPixel(center_x+radius, center_y  , color);
    result = lcd_o_drawPixel(center_x-radius, center_y  , color);
    ...
    while (x<y) {
        ...
        result = lcd_o_drawPixel(center_x + x, center_y + y, color);
        result = lcd_o_drawPixel(center_x - x, center_y + y, color);
        result = lcd_o_drawPixel(center_x + x, center_y - y, color);
        result = lcd_o_drawPixel(center_x - x, center_y - y, color);
        result = lcd_o_drawPixel(center_x + y, center_y + x, color);
        result = lcd_o_drawPixel(center_x - y, center_y + x, color);
        result = lcd_o_drawPixel(center_x + y, center_y - x, color);
        result = lcd_o_drawPixel(center_x - y, center_y - x, color);
    }
...
uint8_t lcd_o_fillCircle(uint8_t center_x, uint8_t center_y, uint8_t radius, uint8_t color) {
    uint8_t result;
    for(uint8_t i=0; i<= radius;i++){
        result = lcd_o_drawCircle(center_x, center_y, i, color);
    }
    return result;
}
uint8_t lcd_o_drawBitmap(uint8_t x, uint8_t y, const uint8_t *picture, uint8_t width, uint8_t height, uint8_t color){
    uint8_t result,i,j, byteWidth = (width+7)/8;
    for (j = 0; j < height; j++) {
        for(i=0; i < width;i++){
            if(pgm_read_byte(picture + j * byteWidth + i / 8) & (128 >> (i & 7))){
                result = lcd_o_drawPixel(x+i, y+j, color);
            } else {
                result = lcd_o_drawPixel(x+i, y+j, !color);
            }
        }
...
#if defined (SSD1306) || defined (SSD1309)
    lcd_o_gotoxy(0,0);
    lcd_o_data(&displayBuffer[0][0], DISPLAY_WIDTH*DISPLAY_HEIGHT/8);
#elif defined SH1106
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        lcd_o_gotoxy(0,i);
        lcd_o_data(displayBuffer[i], sizeof(displayBuffer[i]));
...
void lcd_o_clear_buffer() {
    for (uint8_t i = 0; i < DISPLAY_HEIGHT/8; i++){
        memset(displayBuffer[i], 0x00, sizeof(displayBuffer[i]));
    }
}
uint8_t lcd_o_check_buffer(uint8_t x, uint8_t y) {
...
void lcd_o_display_block(uint8_t x, uint8_t line, uint8_t width) {
    if (line > (DISPLAY_HEIGHT/8-1) || x > DISPLAY_WIDTH - 1){return;}
    if (x + width > DISPLAY_WIDTH) { // no -1 here, x alone is width 1
        width = DISPLAY_WIDTH - x;
    }
    lcd_o_goto_xpix_y(x,line);
    lcd_o_data(&displayBuffer[line][x], width);
}
#endif
