...
#include "i2c.h"	// library for I2C-communication
    ...
    // i2c_start(), i2c_write_byte(), i2c_stop()
...
void lcd_o_init(uint8_t dispAttr);
    ...
    void lcd_o_command(uint8_t cmd[], uint8_t size);    // transmit command to display
    void lcd_o_data(uint8_t data[], uint16_t size);    // transmit data to display
    void lcd_o_init(uint8_t dispAttr);
    void lcd_o_home(void);                        // set cursor to 0,0
    void lcd_o_invert(uint8_t invert);        	// invert display
    void lcd_o_sleep(uint8_t sleep);            	// display goto sleep (power off)
    void lcd_o_set_contrast(uint8_t contrast);    // set contrast for display
    void lcd_o_puts(const char* s);            	// print string, \n-terminated, from ram on screen (TEXTMODE)
    						// or buffer (GRAPHICMODE)
    void lcd_o_puts_p(const char* progmem_s);	// print string from flash on screen (TEXTMODE)
    						// or buffer (GRAPHICMODE)
    
    void lcd_o_clrscr(void);                	// clear screen (and buffer at GRFAICMODE)
    void lcd_o_gotoxy(uint8_t x, uint8_t y);      // set curser at pos x, y. x means character,
    // y means line (page, refer lcd manual)
    void lcd_o_goto_xpix_y(uint8_t x, uint8_t y); // set curser at pos x, y. x means pixel,
    // y means line (page, refer lcd manual)
    void lcd_o_putc(char c);                	// print character on screen at TEXTMODE
    // at GRAPHICMODE print character to buffer
    void lcd_o_charMode(uint8_t mode);            // set size of chars
    void lcd_o_flip(uint8_t flipping);		// flip display, 
    ...
    void lcd_o_display(void);                	// copy buffer to display RAM
    void lcd_o_clear_buffer(void); 		// clear display buffer
    uint8_t lcd_o_check_buffer(uint8_t x, uint8_t y); // read a pixel value from the display buffer
    void lcd_o_display_block(uint8_t x, uint8_t line, uint8_t width); // display (part of) a display line
#endif
